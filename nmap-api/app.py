...
# Red de laboratorio permitida. Configurable via variable de entorno SCAN_NETWORK.
SCAN_NETWORK = os.environ.get("SCAN_NETWORK", "192.168.100.0/24")
_ALLOWED_NET = ipaddress.IPv4Network(SCAN_NETWORK, strict=False)


def is_target_allowed(target: str) -> bool:
    """Valida que el target esté dentro de la red de laboratorio configurada."""
    try:
        if "/" in target:
            target_net = ipaddress.IPv4Network(target, strict=False)
            return target_net.subnet_of(_ALLOWED_NET) or target_net == _ALLOWED_NET
        return ipaddress.IPv4Address(target) in _ALLOWED_NET
    except ValueError:
        return False
