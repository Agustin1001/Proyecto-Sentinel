...
def create_scan_job(target: str, scan_type: str) -> dict:
    ...
    initial_log = f"[SYSTEM] Scan queued for {target}"
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
                """
                INSERT INTO active_scans (scan_id, target, scan_type, status, progress, logs)
                VALUES (%s, %s, %s, 'queued', 5, %s)
                """,
                (scan_id, target, scan_type, initial_log),
            )
        conn.commit()
    return {
        "id":         scan_id,
            ...
            if log:
                cur.execute(
                    ...
                    SET status = %s, progress = %s,
                        logs = CASE WHEN logs = '' THEN %s
                               ELSE logs || chr(10) || %s END
                    WHERE scan_id = %s
                    """,
                    (status, progress, log, log, scan_id),
                )
            else:
                cur.execute(
                    "UPDATE active_scans SET status = %s, progress = %s WHERE scan_id = %s",
...
def get_scan_status(scan_id):
    with get_conn() as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute(
                "SELECT * FROM active_scans WHERE scan_id = %s", (scan_id,)
            )
            row = cur.fetchone()
    if not row:
    ...
    logs_str = job.get("logs") or ""
    job["logs"] = [line for line in logs_str.split("\n") if line]
    job["id"] = job["scan_id"]
    return jsonify(job)
